...
class SlidingWindowRateLimiter:
    ...
    def _cleanup_window(self, user_id: str, current_time: float) -> None:
        """Очищає застарілі запити з вікна та видаляє користувача, якщо його історія порожня."""
        if user_id in self.user_history:
            # Видаляємо всі мітки часу, які вийшли за межі поточного часового вікна
            ...
            # Якщо після очищення черга порожня, видаляємо користувача зі структури даних
            if not self.user_history[user_id]:
                del self.user_history[user_id]

    def can_send_message(self, user_id: str) -> bool:
        """Перевіряє, чи може користувач надіслати повідомлення у поточному вікні."""
        current_time = time.time()
        self._cleanup_window(user_id, current_time)
        
        # Якщо користувача немає в базі, він точно може надіслати повідомлення
    ...
    def time_until_next_allowed(self, user_id: str) -> float:
        ...
        # Якщо ліміт не вичерпано, очікування не потрібне
        if user_id not in self.user_history or len(self.user_history[user_id]) < self.max_requests:
            return 0.0
            
        # Наступний слот звільниться, коли найстаріше повідомлення вийде за межі вікна
